...
void KOMO::setPlace(double time, const char* endeffRef, const char* object, const char* placeRef,  int verbose , const char* bound){
...
//  if(!effKinMode)  setKinematicSwitch(time, true, "rigidAtTo", placeRef, object); //OLD!!
  mlr::Transformation rel = 0;
  rel.addRelativeTranslation( 0.3, 0.3, .25*(height(world.getShapeByName(object)) + height(world.getShapeByName(placeRef)))); //TODO verify correctness of this
  setKinematicSwitch(time, true, "rigidZero", placeRef, object, rel );
}


void KOMO::setPlaceOne(double time, const char* endeffRef, const char* object, const char* placeRef,  int verbose , const char* bound){
  if(verbose>0) cout <<"KOMO_setPlace t=" <<time <<" endeff=" <<endeffRef <<" obj=" <<object <<" place=" <<placeRef <<endl;
...
void KOMO::setPlaceTwo(double time, const char* endeffRef, const char* object, const char* placeRef,  int verbose , const char* bound){
  if(verbose>0) cout <<"KOMO_setPlace t=" <<time <<" endeff=" <<endeffRef <<" obj=" <<object <<" place=" <<placeRef <<endl;
...
void KOMO::setPlaceFloating(double time, const char* endeffRef, const char* object, const char* placeRef,  int verbose , const char* bound){
  if(verbose>0) cout <<"KOMO_setPlace t=" <<time <<" endeff=" <<endeffRef <<" obj=" <<object <<" place=" <<placeRef <<endl;
...
//  setTask(time, time, new TaskMap_Default(posDiffTMT, world, object, NoVector, placeRef, NoVector), sumOfSqrTT, {0.,0.,.1}, 1e-1);
  ...
  //place inside box support
  if(bound){
    setTask(time, time, new TaskMap_AboveBox(world, object, bound), ineqTT, NoArr, 1e2);
  }
  else{
    setTask(time, time, new TaskMap_AboveBox(world, object, placeRef), ineqTT, NoArr, 1e2);
  }
...
//  if(!effKinMode)  setKinematicSwitch(time, true, "rigidAtTo", placeRef, object); //OLD!!
  mlr::Transformation rel = 0;
  rel.addRelativeTranslation( 0., 0., 1.*(height(world.getShapeByName(object)) + height(world.getShapeByName(placeRef)))); //TODO verify correctness of this
...
//      optConstrained(x, dual, Convert(*MP), OPT(verbose=2));
      ...
      opt = new OptConstrained(x, dual, C, OPT(verbose=0));
      opt->run();
#endif
    }else{
      arr a,b,c,d,e;
...
//      ConstrainedProblem P0 = conv_KOrderMarkovFunction2ConstrainedProblem(*MP);
  ...
  if(verbose>1) cout <<MP->getReport(false, fileId);
}

Graph KOMO::getReport(bool gnuplt, int fileId ){
  return MP->getReport(gnuplt,  fileId);
}
...
#if 1
  return MP->displayTrajectory(watch?-1:1, "", delay);
#else
  if(MP->T){
    ::displayTrajectory(x, 1, world, MP->switches, "", delay);
  //  orsDrawProxies=true;
