...
struct MotionProblem {
  ...
  //-- info on the costs
  Graph getReport(bool gnuplt=true, int fileId=1); ///< return summary of costs and constraint violations for all features
  void costReport(bool gnuplt=true, int fileId=1){ cout <<getReport(gnuplt, fileId) <<endl; } //old convention
