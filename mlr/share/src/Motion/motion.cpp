...
Graph MotionProblem::getReport(bool gnuplt, int fileId) {
  ...
  double totalSqrCostUp = 0.;
  for(uint i=0; i<tasks.N; i++) {
    Task *c = tasks(i);
    Graph *g = &report.newSubgraph({c->name}, {})->value;
    cout << "THE COSTS ########################################" << endl;
    cout <<  c->name << ":" << taskC(i)<<  endl;
  
    if(!(c->name.contains("TaskMap_Proxy_"))){
      //cout << "TASK NAME IS " << c->name << endl;
      totalSqrCostUp += taskC(i);
    }

    
    g->newNode<double>({"order"}, {}, c->map.order);
    g->newNode<mlr::String>({"type"}, {}, STRING(TermTypeString[c->type]));
  ...
  ofstream fol(STRING("sqrCostUp"<<fileId<<".txt"));
  fol << totalSqrCostUp << endl;
  fol.close();
  ...
  for(auto c:tasks) if(c->type==ineqTT && dualSolution.N) fil <<c->name <<"_dual ";
  fil <<endl;
